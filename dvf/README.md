...
uv pip install -r /src/dvf/requirements.txt 
...
It's all run from `dvf.py`:

```bash
# Basic run (prints errors to console)
python -m dvf --config_file test.yaml --data_file test.csv

# Run with performance profiling (prints profile to console)
python -m dvf --config_file test.yaml --data_file test.csv --profile
...
python -m dvf --config_file test.yaml --data_file test.csv --save_results

# Run and save results to a *specific* file
python -m dvf --config_file test.yaml --data_file test.csv --save_results output/my_report.txt

# Run with all options enabled
python -m dvf --config_file test.yaml --data_file test.csv --save_results --profile
```
...
1.  Create a new `MyCoolReader.py` in `src/utils/readers/`.
...
3.  Add it to the `_readers` dictionary in `src/utils/readers/reader_factory.py`.

### 2. How to add a new config setting:

1.  Add the new field to the `Config` class in `src/dvf/config/config_model.py`.
...
1.  Create your `MyNewValidator.py` in `src/dvf/validations/`.
